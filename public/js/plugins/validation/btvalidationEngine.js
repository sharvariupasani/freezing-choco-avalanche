 ...
 */
 (function($) {
	 ...
	 var methods = {
		...
		*/
		 ...
		 */
		 hideAll: function() {
			 ...
			 $('.validation').fadeTo(options.fadeDuration, 0.3, function() {
				  $(this).parent('.formErrorOuter').remove();
				 $(this).remove();
			 });
			 return this;
		...
		_validateFields: function(form, skipAjaxValidation) {
			...
			if (errorFound) {
				if (options.scroll) {
					...
					if (positionType!="bottomRight" && positionType!="bottomLeft") {
					var prompt_err= methods._getPrompt(first_err);
					if(prompt_err != undefined) destination=prompt_err.offset().top;
					}
			...
			for (var i = 0; i < rules.length; i++) {
				...
				switch (rules[i]) {
					...
					case "max":
						...
						break;
					case "compare":
						errorMsg = methods._compare(field, rules, i, options);
						break;
					case "past":
		...
		* @return an error string if validation failed
		*/
		_compare: function(field, rules, i, options) {
			var equalsField = rules[i + 1];
			
			if(parseInt(money2num($("#" + equalsField).val())) > parseInt(money2num(field.val()))) {
				return options.allrules.compare.alertText;
			}			
		},
		/**
		...
		*/
		_buildPrompt: function(field, promptText, type, ajaxed, options) {
			...
			//prompt.prepend("<a class=\"close\" data-dismiss=\"alert\">x</a>");
