/*Added & Modified By NIRAV for responsive layout*/
...
self.render_layout = function render_layout() {
  // recreates the columns when a media query matches the current state
  // of the browser.
    Array.prototype.forEach.call(grids, self.recreate_columns);
};
...
self.next_element_column_index = function next_element_column_index(grid) {
 ...
 for (i=0;i<children.length;i++)
 {
	 ...
	 rowCount.push(currentRowCount);
 }
 var i = rowCount.indexOf(Math.min.apply(Math, rowCount));
 /*
  var rowCount = []; 
  for (i; i >= 0; i--) {
    child = children[i];
