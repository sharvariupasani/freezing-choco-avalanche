...
$.TextboxList = function(element, _options){
        ...
        $(document).click(function(e){
            ...
            switch (ev.which){
                case 8:
                    ...
                    if (current.is('editable') && caret == 0 && !value.length){
                        evStop();
                        var prev = getRelative('prev');
                        if(prev) {
                            return prev.remove()
                        }
                    }
    ...
    var getRelative = function(dir) {
        var el = getBit(current).toElement();
        var b = getBit(el[dir]());
        return b;
    }
	...
	var clearValues = function() {
	   ...
	   index = []; // Added BY NIRAV: Check for unique tag functionalities.
   };
    ...
    this.onRemove = function(bit){
       ...
       // focusRelative('next', bit);	// by Prakash
    };
...
$.TextboxListBit = function(type, value, textboxlist, _options){
        ...
        if (type == 'box'){
            ...
            element.focus(function(){ focus(true); }).blur(function(){
                ...
                if (options.addOnBlur)
                    {
                        if(self.getValue()[1].length > 1) {
                            toBox();
                        }

                    }
            });
            if (options.addKeys || options.stopEnter || options.quoteStr ){
                element.keydown(function(ev){
                    if (!focused) return;
                    var evStop = function(){ ev.stopPropagation(); ev.preventDefault(); };
                    if (options.stopEnter && ev.which === 13)
                        {
                            evStop();
                        }
                    ...
                    if (options.quoteStr) {
                        var spaceKey = 32;
                        var quoteKey = 222;
                        var curValue = self.getValue();
                        var haveQuote = false;

                        if (curValue[1][0] == "'" || curValue[1][0] == "\"")
                        {
                                haveQuote = true;
                        }


                        if(ev.which == quoteKey)
                        {
                            if(haveQuote)
                            {
                                element.val(curValue[1].substr(1));
                                evStop();
                                toBox();
                            }
                        }
                        else if(ev.which == spaceKey)
                        {
                            if(haveQuote == false)
                            {
                                evStop();
    ...
    var fireBitEvent = function(type){
        ...
        textboxlist.fireEvent('bit' + type, self);
        textboxlist.fireEvent('bit' + name + type, self);
        return self;
    };
