...
// TODO: use uname -m or -a
        ...
        protected static function _getCMD(){
                if(self::$_cmd==''){
                        ...
                        if(self::_getOS()=='win'){
                                ...
                                self::$_cmd=$GLOBALS['WKPDF_BASE_PATH'].'wkhtmltopdf-'.self::_getCPU();
                        }
                }
                switch(self::_getOS()){
        ...
        public function set_html($html){
                ...
                do{
						$tmp_path = realpath("./uploads/").'/pdftmp';
                ...
                $this->url=base_url()."uploads/pdftmp/".basename($this->tmp);
        }
        /**
...
class WKPDF_MULTI extends WKPDF {
        ...
        public function add_html($html){
                do{
						$tmp_path = realpath("./uploads/").'/pdftmp';exit;
						if (!file_exists($tmp_path)) {
							mkdir($tmp_path, 0777, true);
						}
                        $file=$tmp_path.'/'.mt_rand().'.html';
                } while(file_exists($file));
                if(!file_put_contents($file,$html))throw new Exception('WKPDF write temporary file failed.');
                $this->html_urls[]=public_path()."pdftmp/".basename($this->tmp);
